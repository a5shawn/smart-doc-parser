...
- **结果可信度可见** —— 每个结果都带 `warnings`：哪些必填字段没抽到、哪些值被自动转换、
  文档是否因过长被截断。不把问题藏起来
- **实时进度** —— SSE 推送阶段变化与**模型逐字输出**。推理模型思考要 1 秒以上，
  流式让等待可感知
...
4. 约 2 秒后完成，字段按中文标签排布成卡片（甲方 / 乙方 / 合同金额…）
...
| 指标 | 实测值 |
|---|---|
| 单份合同抽取耗时 | **约 2 秒**（其中思考阶段约 1.2~1.5 秒） |
| 单份合同成本 | **约 ¥0.004**（输入约 1300 token + 输出约 250 token） |
| 1000 份合同成本 | 约 ¥4 |
| 前缀缓存命中 | 相同模板重复抽取时命中率可达 60%+，成本进一步下降 |
| 抽取准确率 | 内置的三份示例文档上，必填字段全部正确，无告警 |

**为什么这是"分水岭"**：把成本算出来、记进数据库、显示在界面上，
